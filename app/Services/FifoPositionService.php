...
class FifoPositionService
{
    ...
    private function processSell(Fill $sellFill, $buyQueue, Instrument $instrument): void
    {
        ...
        while ($remainingSellQty > 0 && $buyQueue->isNotEmpty()) {
            ...
            $buyTurnover = $buyPrice * $multiplier * $matchQty;
            $sellTurnover = $sellPrice * $multiplier * $matchQty;
            $brokerageFees = 0;
            if ($brokerageRate > 0) {
                $brokerageFees = round(($buyTurnover + $sellTurnover) * ($brokerageRate / 100), 2);
            }
            $totalFees = $buyFees + $sellFees + $brokerageFees;
